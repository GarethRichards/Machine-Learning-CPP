...
namespace NeuralNet {
	...
	template <typename T> void sigmoid(ublas::vector<T> &v) {
		for (auto &iv : v) {
			iv = 1.0 / (1.0 + exp(-iv));
		}
	}
	// Derivative of the sigmoid function.
	template <typename T> void sigmoid_prime(ublas::vector<T> &v) {
		for (auto &iv : v) {
			iv = 1.0 / (1.0 + exp(-iv));
			iv = iv * (1.0 - iv);
		}
	...
	template <typename T, typename CostPolicy> class Network : private CostPolicy {
	private:
	...
	public:
		Network(const std::vector<int> &sizes) : m_sizes(sizes) {
		...
		void SGD(TrainingDataIterator td_begin, TrainingDataIterator td_end, int epochs, int mini_batch_size, T eta,
			T lmbda, std::function<void(const Network &, int Epoc)> feedback) {
		...
		void backprop(const ublas::vector<T> &x, const ublas::vector<T> &y, std::vector<ublas::vector<T>> &nabla_b,
			std::vector<ublas::matrix<T>> &nabla_w) {
			auto activation = x;
